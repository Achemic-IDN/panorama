...
class QueueSocketClient {
  ...
  setupEventHandlers() {
    ...
    // Generic monitoring-friendly event alias
    this.socket.on('queue:update', (event) => {
      this.emit('queue:update', event);
    });
  ...
  enablePollingFallback() {
    ...
    this.pollingInterval = setInterval(async () => {
      try {
        const url = this.role === "MONITORING" ? "/api/monitor/queues" : "/api/admin/queue";
        const response = await fetch(url, { cache: 'no-store' });
        if (response.ok) {
          const json = await response.json().catch(() => null);
          const queues = Array.isArray(json) ? json : json?.data?.queues || json?.queues || [];
