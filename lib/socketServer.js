...
function getRoomsForStatus(status) {
  const rooms = [ROOMS.UTAMA, ROOMS.MONITORING]; // UTAMA + monitoring see all
...
export function broadcastQueueCreated(queue) {
  ...
  io.to(ROOMS.MONITORING).emit('queue:created', event);
  io.to(ROOMS.MONITORING).emit('queue:update', event);
}
...
export function broadcastQueueCompleted(queue) {
  ...
  io.to(ROOMS.MONITORING).emit('queue:completed', event);
  io.to(ROOMS.MONITORING).emit('queue:update', event);
